...
def main():
    ...
    markers = make_markers_from_vcf_file(bed_file, gzipped)
        ...
        for chrm_k in make_chrm_list():
            ...
            for seg in segs:
                ...
                for marker in chrm_markers:
                    if int(marker) >= int(seg[0]) and int(marker) <= int(seg[1]):
                        marker_count += 1
...
def make_markers_from_vcf_file(vcf_file, gzipped):
    vcf_markers = {chrm : set() for chrm in make_chrm_list()}

    if gzipped:
        vcf_file = gzip.open(vcf_file, mode = 'rt')
    else:
        vcf_file = open(vcf_file, mode = 'r')
    ...
    for line in vcf_file:
        ...
        if i < 10000:
            ...
            continue
        i = 0
        chrm, start = line.split('\t')[0:2]
        if not chrm.startswith("chr"):
            chrm = "chr" + chrm
        if chrm not in make_chrm_list():
            continue

        vcf_markers[chrm].add(start)
...
def parse_args():
    ...
    parser.add_argument('-b', '--bed_file', required=True,
                        help='Specify BED file for marker output.')
    parser.add_argument('-g', '--gzipped', action = 'store_true')
